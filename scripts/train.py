...
def startup(fabric: Fabric, cfg: CLISettings):
    ...
    # Phase 1 of FP-only fine-tuning: freeze the backbone before the
    # optimizer is built so frozen params are excluded from param groups.
    _apply_freeze_backbone(fabric, model, cfg)
    ...
    load_checkpoint(fabric, state, cfg.out_dir, cfg.run_name, cfg.model_checkpoint,
                    cfg.resume, cfg.resume_checkpoint_path,
                    fp_only_load=bool(getattr(cfg, "freeze_backbone", False)))

    # Phase 2 of FP-only fine-tuning: re-init FP-block 2D weights AFTER the
    # checkpoint load so the loaded values aren't used in place of our init.
    _apply_reinit_fp_weights(fabric, model, cfg)
...
def train_step(input_ids, labels, fabric, state, running_loss, running_ppl, cfg):
    ...
    if not state["is_accumulating"]:
        ...
        if state["base_muon_wd"] > 0 and cfg.max_steps > 0:
            ...
            for param_group in optimizer.param_groups:
                if param_group.get("kind") == "muon" and not param_group.get("pin_wd", False):
...
def train(fabric, state, cfg):
    ...
    for input_ids, labels, _ in train_iterator:
        ...
        if validate_regular or validate_at_the_end:
            ...
            cur_val = float(val_metrics["val_loss"].item())
            best_so_far = float(state.get("best_val_loss", float("inf")))
            if cur_val < best_so_far:
                state["best_val_loss"] = cur_val
                prefix = fabric.get_prefix_for_checkpoint()
                best_path = f"{cfg.out_dir}/{prefix}/best-{cfg.run_name}"
                fabric.print(f"New best val loss {cur_val:.4f} (was {best_so_far:.4f}); saving to {best_path!r}")
                fabric.save(best_path, state)
...
def _fp_only_inner(model):
    inner = model.module if hasattr(model, "module") else model
    inner = inner._orig_mod if hasattr(inner, "_orig_mod") else inner
    if (hasattr(inner, "transformer")
            and hasattr(inner.transformer, "core_block")
            and inner.transformer.core_block is not None):
        return inner
    return None


def _apply_freeze_backbone(fabric, model, cfg):
    """Phase 1 of FP-only fine-tuning: freeze every param outside
    ``transformer.core_block`` so the optimizer (built right after this) skips
    them. Must run BEFORE optimizer construction. ``reinit_fp_weights`` is
    deferred to phase 2 (after load_checkpoint) so the checkpoint load doesn't
    overwrite our fresh init.
    """
    if not getattr(cfg, "freeze_backbone", False):
        return
    inner = _fp_only_inner(model)
    if inner is None:
        fabric.print("[fp-only] model has no core_block; skipping freeze.")
        return
    ...
    for name, p in inner.named_parameters():
        if "core_block" in name:
            ...
            n_frozen += p.numel()
    fabric.print(
        f"[fp-only] frozen={n_frozen/1e6:.1f}M  "
        f"trainable(core_block)={n_trainable/1e6:.1f}M")


def _apply_reinit_fp_weights(fabric, model, cfg):
    """Phase 2 of FP-only fine-tuning: re-initialise FP-block 2D weights.
    Runs AFTER ``load_checkpoint`` so the loaded (init-level) FP weights are
    replaced with a more useful normal init."""
    if not getattr(cfg, "reinit_fp_weights", False):
        return
    inner = _fp_only_inner(model)
    if inner is None:
        fabric.print("[fp-only] model has no core_block; skipping reinit.")
        return

    std = getattr(cfg, "reinit_fp_std", None)
    if std is None:
        n_embd = getattr(inner.config, "n_embd", None)
        std = (1.0 / math.sqrt(n_embd)) if n_embd else 0.02
    else:
        std = float(std)
    fabric.print(f"[fp-only] re-initialising FP-block 2D weights with std={std:.4g}")
    with torch.no_grad():
        for name, p in inner.transformer.core_block.named_parameters():
            if p.ndim >= 2:
                p.normal_(mean=0.0, std=std)


def load_checkpoint(fabric, state, out_dir, run_name, model_checkpoint, resume=True, resume_checkpoint_path=None, fp_only_load=False):
    resume_ckpt = None
    t0 = time.time()
    if resume:
    ...
    if resume_ckpt is None and model_checkpoint is not None:
        ...
        # FP-only fine-tuning: load JUST the model weights and skip optimizer
        # / dataloader / step counters, since the fresh optimizer has a
        # different number of param groups than the source checkpoint.
        if fp_only_load:
            fabric.print(f"Loaded MODEL WEIGHTS ONLY (FP-only fine-tuning) from {model_checkpoint}")
            sd = checkpoint_raw.get("model", checkpoint_raw)
            if hasattr(sd, "state_dict"):
                sd = sd.state_dict()
            cleaned = {}
            for k, v in sd.items():
                for pfx in ("module.", "_orig_mod.", "model.", "_forward_module."):
                    if k.startswith(pfx):
                        k = k[len(pfx):]
                cleaned[k] = v
            target = state["model"]
            inner = target.module if hasattr(target, "module") else target
            inner = inner._orig_mod if hasattr(inner, "_orig_mod") else inner
            missing, unexpected = inner.load_state_dict(cleaned, strict=False)
            fabric.print(f"  Loaded weights: missing={len(missing)} unexpected={len(unexpected)}")
            if missing[:3]:
                fabric.print(f"    missing[:3]: {missing[:3]}")
        else:
...
def maybe_save_checkpoint(fabric, state, cfg, is_accumulating=False, force_save=False):
    t0 = time.time()
    prefix = fabric.get_prefix_for_checkpoint()
    # Save to a fixed `last` path, overwriting — we keep only best + last.
    fully_qualified_checkpoint_path = f"{cfg.out_dir}/{prefix}/last-{cfg.run_name}"
